...
//    @PreAuthorize("@ss.hasPermi('book:BookInfo:list')")
     ...
     */
//    @PreAuthorize("@ss.hasPermi('book:BookInfo:list')")
    @GetMapping("/list/title/all")
    @Anonymous
    public AjaxResult listAllTitle() {
        List<Long> availableBookIDsList = bookStorageService.selectAvailableBookIDsList();
        List<Books> availableBooksList = booksService.selectBooksListByIds(availableBookIDsList);
        List<String> bookNameList = availableBooksList.stream().map(Books::getTitle).collect(Collectors.toList());
        return AjaxResult.success(bookNameList);
    }
...
//    @PreAuthorize("@ss.hasPermi('book:BookInfo:remove')")
    ...
    @Anonymous
    public AjaxResult handleExtension(@RequestBody BorrowBO request) {
