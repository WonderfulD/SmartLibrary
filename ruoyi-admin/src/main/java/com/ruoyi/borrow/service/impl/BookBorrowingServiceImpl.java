...
import com.ruoyi.borrow.domain.BorrowBO;
...
public class BookBorrowingServiceImpl implements IBookBorrowingService {
    ...
    @Override
    @Transactional
    public AjaxResult handleExtension(BorrowBO request) {
        if (!checkExtensionInput(request)) {
            return AjaxResult.error("入参非法");
        }
        BookBorrowing borrowRecord = selectBookBorrowingByBorrowId(request.getBorrowId());
        if (ObjectUtil.isEmpty(borrowRecord)) {
            return AjaxResult.error("操作失败，该借阅记录不存在");
        }
        // 检查图书是否已归还
        ...
        try {
            Long borrowId = borrowRecord.getBorrowId();
            ...
            //检查是否已有延期
            List<BookBorrowExtension> extensionList =
                    bookBorrowExtensionService.selectBookBorrowExtensionList(buildBorrowExtensionQuery(borrowRecord));
            try {
                ifExtensionCanProceed(extensionList);
            } catch (Exception e) {
            ...
            int extendedDays;
            // 判断是否传入延期日期
            if (request.getExtensionDate() != null) {
                extensionDate = request.getExtensionDate();
                extendedDays = DateUtil.calculateDateDifference(borrowRecord.getDueDate(), extensionDate);
            } else {
                extensionDate = dueDate.plusDays(15);
                extendedDays = 15;
            }
            // 创建并修改借阅记录
            ...
            bookBorrowing.setComments("延期" + extendedDays + "天成功");
            updateBookBorrowing(bookBorrowing);

            // 创建延期记录
            BookBorrowExtension bookBorrowExtension =
                    buildExtensionRecord(borrowRecord, request, extendedDays, extensionDate);

            bookBorrowExtensionService.insertBookBorrowExtension(bookBorrowExtension);
    ...
    private BookBorrowExtension buildExtensionRecord(BookBorrowing bb, BorrowBO request, Integer extendedDays, LocalDate extensionDate) {
        BookBorrowExtension bookBorrowExtension = new BookBorrowExtension();
        bookBorrowExtension.setBorrowId(bb.getBorrowId());
        bookBorrowExtension.setBookId(bb.getBookId());
        bookBorrowExtension.setStatus(2);
        bookBorrowExtension.setApprovalComment("延期" + extendedDays + "天成功");
        bookBorrowExtension.setRequestTime(LocalDateTime.now());
        bookBorrowExtension.setApprovalTime(LocalDateTime.now());
        bookBorrowExtension.setRequestedDueTime(request.getExtensionDate());
        bookBorrowExtension.setApprovedDueTime(extensionDate);
        bookBorrowExtension.setGmtCreated(LocalDateTime.now());
        return bookBorrowExtension;
    }

    private void ifExtensionCanProceed(List<BookBorrowExtension> recordlist) {
        if (CollUtil.isEmpty(recordlist)) {
            return;
        }
        BookBorrowExtension record = recordlist.get(0);
        if (record.getStatus() == 0) {
            throw new RuntimeException("操作失败，该借阅记录存在一个正在进行中的延期申请");
        }
        if (record.getStatus() == 1) {
            throw new RuntimeException("操作失败，该借阅记录存在一个被拒绝的延期申请，无法再次申请");
        }
        if (record.getStatus() == 2) {
            throw new RuntimeException("操作失败，你已经申请过延期");
        }
    }

    private BookBorrowExtension buildBorrowExtensionQuery(BookBorrowing bb) {
        BookBorrowExtension query = new BookBorrowExtension();
        query.setBorrowId(bb.getBorrowId());
        return query;
    }

    private Boolean checkExtensionInput(BorrowBO bb) {
        return !ObjectUtil.isEmpty(bb) && bb.getBorrowId() != null;
    }
